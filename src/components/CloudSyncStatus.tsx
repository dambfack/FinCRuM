...
import { Cloud, CloudOff, RefreshCw, CheckCircle, AlertCircle, Clock, AlertTriangle } from 'lucide-react';
...
export function CloudSyncStatus({ className, showDetails = true }: CloudSyncStatusProps) {
        ...
        <CardContent className="space-y-4">
          ...
          {!state.isConnected && (
            <Alert>
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>
                Connect to Google Drive or OneDrive in Settings to enable cloud synchronization.
              </AlertDescription>
            </Alert>
          )}
        </CardContent>
 ...
 */
export function CloudSyncStatusCompact({ className }: { className?: string }) {
  return <CloudSyncStatus className={className} showDetails={false} />;
}