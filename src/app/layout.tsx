...
function AppContent({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (storedCustomBg) applyInitialBackground(storedCustomBg);
      ...
      <SidebarInset className="flex flex-col">
        <header className={cn("sticky top-2 z-20 flex h-16 items-center justify-between px-4 md:px-6 mx-2 md:mx-4 rounded-lg glass-effect bg-background/50 dark:bg-background/40 hover:shadow-2xl transition-shadow duration-300")}>
            ...
            {!isMobileSidebarOpen && ( 
              <Link href="/" className="font-semibold text-lg flex items-center gap-2 text-foreground hover:text-primary transition-colors" aria-label="View dashboard">
                <div className="flex items-center font-heading">
                  {currentHeaderLogoToDisplay ? <NextImage src={currentHeaderLogoToDisplay} alt="Header Logo" width={256} height={40} className="h-10 w-auto max-w-xs mr-1 object-contain" data-ai-hint="custom header logo mobile" unoptimized/> : <span className="mr-1">Finsculpt</span>}
                  <span>CRM</span>
                </div>
              </Link>
            )}
          ...
          <div className="hidden md:flex items-center text-xl font-semibold font-heading">
            {currentHeaderLogoToDisplay ? <NextImage src={currentHeaderLogoToDisplay} alt="Header Logo" width={256} height={40} className="h-10 w-auto max-w-xs mr-1 object-contain" data-ai-hint="custom header logo" unoptimized/> : <span className="mr-1">Finsculpt</span>}
            <span>CRM</span>
          </div>
